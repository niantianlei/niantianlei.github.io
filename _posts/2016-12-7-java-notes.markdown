...
==:若两个变量是基本类型变量，则比较数值；若是引用类型变量，则检查是否指向同意对象。  
equals:若类没有重写该方法则和==一样，判断是否指向同一对象。
...
final修饰的成员变量必须在声明的同时赋值，如果在声明的时候没有赋值，那么只有； 一次赋值的机会，而且只能在构造方法中显式赋值，然后才能使用；  
